...
import { safeAuthUrl, safeEqual } from '../lib/auth-policy';
...
export async function getAppUser():Promise<AppUser|null>{
  ...
  const trustChatGPT=mode==='chatgpt'||process.env.AUTH_TRUST_CHATGPT_HEADERS==='true';
  if(trustChatGPT&&(mode==='auto'||mode==='chatgpt')){
...
export function getAuthOptions():AuthOption[]{return[
  ...
  {id:'chatgpt',label:'ChatGPT',description:'适合 OpenAI Sites，自动获得个人云同步。',available:process.env.AUTH_ENABLE_CHATGPT==='true',href:'/signin-with-chatgpt?return_to=/'},
