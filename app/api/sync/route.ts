...
export async function GET() {
  ...
  if (!user) return Response.json({authenticated:false,authOptions:getAuthOptions()});
  try {
    await ensureSchema();
    ...
    return Response.json({authenticated:true,user,authOptions:getAuthOptions(),state:row?JSON.parse(row.state_json):null,version:row?.version??0,updatedAt:row?.updated_at??null});
  } catch {
    return Response.json({ authenticated:true, user, authOptions:getAuthOptions(), state:null, version:0, cloudUnavailable:true });
  }
}
